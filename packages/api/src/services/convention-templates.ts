/**
 * Convention Starter Templates
 *
 * Preset coding convention templates that users can choose from when setting
 * up their repo. Each template contains the key coding rules an LLM needs
 * to write correct code in a given language/framework.
 *
 * These are injected into LLM prompts via the {{conventions}} template variable.
 */
...
export const CONVENTION_TEMPLATES: Record<string, ConventionTemplate> = {
  'typescript-node': {
...
/**
 * Look up a convention template by key.
 */
export function getConventionTemplate(key: string): ConventionTemplate | undefined {
  return CONVENTION_TEMPLATES[key];
}

/**
 * List all available convention templates (key, name, description — no conventions body).
 */
export function listConventionTemplates(): Array<{ key: string; name: string; description: string }> {
  return Object.entries(CONVENTION_TEMPLATES).map(([key, template]) => ({
    key,
    name: template.name,
