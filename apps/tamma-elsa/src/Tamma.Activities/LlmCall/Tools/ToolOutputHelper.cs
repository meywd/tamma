...
public static class ToolOutputHelper
{
    ...
    /// <summary>
    /// Redact common secret patterns from output (API keys, tokens, passwords).
    /// </summary>
    ...
    public static string RedactSecrets(string output)
    {
        ...
        // Common patterns: API keys, Bearer tokens, passwords in connection strings
        var redacted = System.Text.RegularExpressions.Regex.Replace(
            output,
            @"(?i)(api[_-]?key|token|password|secret|credential|authorization:\s*bearer)\s*[=:]\s*\S+",
            "$1=[REDACTED]");
