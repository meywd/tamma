...
const WORKFLOW_DIAGRAMS: Record<string, WorkflowDef> = {
  ...
  'adl-orchestrator': {
    nodes: [
      ...
      n('config', 'Load Config', 0, 0),
      n('limits', 'Check Limits', 0, 0),
      n('limitsOk', 'Within Limits?', 0, 0, 'decision'),
      n('dispatch', 'Dispatch Issue Cycle', 0, 0, 'subworkflow', { description: 'SingleIssueCycleWorkflow' }),
      n('parse', 'Parse Result', 0, 0),
      n('continue', 'More Issues?', 0, 0, 'decision'),
      n('cooldown', 'Cooldown', 0, 0, 'process', { description: '10s between cycles' }),
      n('outputLimits', 'Output (Limits)', 0, 0),
      n('outputNoIssues', 'Output (No Issues)', 0, 0),
      n('done', 'Complete', 0, 0, 'end'),
    ],
    edges: [
      e('start', 'config'),
      e('config', 'limits'),
      e('limits', 'limitsOk'),
      e('limitsOk', 'dispatch', 'True'),
      e('limitsOk', 'outputLimits', 'False', 'right'),
      e('dispatch', 'parse'),
      e('parse', 'continue'),
      e('continue', 'cooldown', 'True'),
      e('continue', 'outputNoIssues', 'False', 'right'),
      e('cooldown', 'limits'),
      e('outputLimits', 'done'),
      e('outputNoIssues', 'done'),
    ],
  },
  ...
  'single-issue-cycle': {
    nodes: [
      n('start', 'Init Config', 280, 0, 'start'),
      n('select', 'Select Issue', 280, 80, 'subworkflow'),
      n('hasIssue', 'Issue Found?', 280, 170, 'decision'),
      n('context', 'Gather Context', 280, 270, 'subworkflow'),
      n('plan', 'Generate Plan', 280, 370, 'subworkflow'),
      n('planApproved', 'Plan Approved?', 280, 460, 'decision'),
      n('branch', 'Create Branch', 280, 560, 'subworkflow'),
      n('branchOk', 'Branch Created?', 280, 650, 'decision'),
      n('tdd', 'TDD with Debug Retry', 280, 750, 'subworkflow'),
      n('tddOk', 'TDD Passed?', 280, 840, 'decision'),
      n('pr', 'Create PR', 280, 940, 'subworkflow'),
      n('prOk', 'PR Created?', 280, 1030, 'decision'),
      n('ci', 'CI with Debug Retry', 280, 1130, 'subworkflow'),
      n('ciOk', 'CI Passed?', 280, 1220, 'decision'),
      n('review', 'Review Fix Check', 280, 1320, 'subworkflow'),
      n('hasComments', 'Has Comments?', 280, 1410, 'decision'),
      n('mergeApproval', 'Merge Approval', 280, 1510, 'subworkflow'),
      n('mergeDecision', 'Merge Approved?', 280, 1600, 'decision'),
      n('testDecision', 'Run More Tests?', 530, 1600, 'decision'),
      n('merge', 'Merge PR', 280, 1700, 'subworkflow'),
      n('mergeOk', 'Merged?', 280, 1790, 'decision'),
      n('finish', 'Shared Finish', 280, 1890),
      n('done', 'Complete', 280, 1970, 'end'),
      n('noIssues', 'No Issues', 530, 170, 'end'),
      n('planRejected', 'Plan Rejected', 530, 460, 'end'),
      n('error', 'Error', 530, 650, 'end'),
      n('tddFailed', 'TDD Failed', 530, 840, 'end'),
      n('ciFailed', 'CI Failed', 530, 1220, 'end'),
      n('reviewRejected', 'Review Rejected', 730, 1600, 'end'),
      n('mergeFailed', 'Merge Failed', 530, 1790, 'end'),
    ],
    edges: [
      e('start', 'select'),
      e('select', 'hasIssue'),
      e('hasIssue', 'context', 'True'),
      e('hasIssue', 'noIssues', 'False', 'right'),
      e('context', 'plan'),
      e('plan', 'planApproved'),
      e('planApproved', 'branch', 'True'),
      e('planApproved', 'planRejected', 'False', 'right'),
      e('branch', 'branchOk'),
      e('branchOk', 'tdd', 'True'),
      e('branchOk', 'error', 'False', 'right'),
      e('tdd', 'tddOk'),
      e('tddOk', 'pr', 'True'),
      e('tddOk', 'tddFailed', 'False', 'right'),
      e('pr', 'prOk'),
      e('prOk', 'ci', 'True'),
      e('prOk', 'error', 'False', 'right'),
      e('ci', 'ciOk'),
      e('ciOk', 'review', 'True'),
      e('ciOk', 'ciFailed', 'False', 'right'),
      e('review', 'hasComments'),
      e('hasComments', 'ci', 'True'),
      e('hasComments', 'mergeApproval', 'False'),
      e('mergeApproval', 'mergeDecision'),
      e('mergeDecision', 'merge', 'True'),
      e('mergeDecision', 'testDecision', 'False', 'right'),
      e('testDecision', 'ci', 'True'),
      e('testDecision', 'reviewRejected', 'False', 'right'),
      e('merge', 'mergeOk'),
      e('mergeOk', 'finish', 'True'),
      e('mergeOk', 'mergeFailed', 'False', 'right'),
      e('finish', 'done'),
    ],
  },
  ...
  'llm-call': {
    nodes: [
      ...
      n('budget', 'Setup Budget', 250, 90),
      n('agent', 'Resolve Agent Config', 250, 180),
      n('chain', 'Resolve Provider Chain', 250, 270),
      n('foreach', 'For Each Provider', 250, 370, 'process', { description: 'Circuit breaker + budget + retry loop' }),
      n('succeeded', 'Call Succeeded?', 250, 480, 'decision'),
      n('outputs', 'Set Outputs', 250, 590),
      n('done', 'Done', 250, 680, 'end'),
      n('failure', 'Build Failure Output', 500, 480),
    ],
    edges: [
      e('start', 'budget'),
      e('budget', 'agent'),
  ...
  'mentorship': {
    nodes: [
      ...
      n('monReview', 'Monitor Review', 250, 1240),
      n('merge', 'Merge and Complete', 250, 1350),
      n('report', 'Generate Report', 250, 1440),
      n('profile', 'Update Skill Profile', 250, 1530),
      n('done', 'Completed', 250, 1620, 'end'),
      n('diagnose', 'Diagnose Blocker', 520, 770, 'subworkflow'),
      n('hint', 'Provide Hint', 520, 880, 'process', { description: 'Socratic' }),
      n('guidance', 'Provide Guidance', 520, 990),
      n('assistance', 'Provide Assistance', 520, 1090),
      n('escalate', 'Escalate to Senior', 520, 1200, 'end'),
      n('autofix', 'Auto-Fix Issues', 520, 400),
      n('guideFixes', 'Guide Fixes', 520, 1350),
  ...
  'code-review': {
    nodes: [
      ...
      n('merge', 'Merge and Complete', 250, 420),
      ...
      n('maxIter', 'Max Iterations?', 520, 710, 'decision'),
      n('escalateMax', 'Escalate: Max Iterations', 750, 710, 'end'),
      n('escalateTimeout', 'Escalate: Timeout', 750, 300, 'end'),
  ...
  'testing': {
    nodes: [
      ...
      n('checks', 'Quality Checks', 250, 300, 'parallel', { items: ['Coverage', 'Linting', 'Security'] }),
      n('report', 'Generate Quality Report', 250, 430),
      n('pass', 'Tests Passed', 250, 530, 'end'),
      n('guard', 'Fix Attempts Remaining?', 530, 190, 'decision'),
      n('fix', 'Commit Auto-Fix', 530, 300),
      n('index', 'Update Code Index', 530, 390),
      n('retrigger', 'Re-Trigger CI', 530, 480),
      n('waitRetry', 'Wait for CI (Retry)', 530, 570, 'process', { description: 'Bookmark' }),
      n('evalRetry', 'Evaluate Retry Results', 530, 660),
      n('fail', 'Tests Failed', 530, 100, 'end'),
      n('critChecks', 'Quality Checks (Critical)', 0, 300, 'parallel', { items: ['Coverage', 'Linting', 'Security'] }),
      n('critReport', 'Generate Report (Critical)', 0, 430),
      n('critFail', 'Tests Failed (Critical)', 0, 530, 'end'),
    ],
    edges: [
      e('start', 'wait'),
      e('wait', 'evaluate'),
  ...
  'debugging': {
    nodes: [
      ...
      n('fork', 'Context Fork', 250, 190, 'parallel', { items: ['Errors', 'Code', 'Git', 'Tests', 'Repro'] }),
