---
title: "Epic 18: End-User Authentication & Registration for Tamma SaaS"
---
...
| 18-3 | Organization/tenant creation | L (5 days) | 18-2 |
| 18-4 | GitHub App installation onboarding | M (3 days) | 18-3 |
| 18-5 | User-facing dashboard shell | L (5 days) | 18-2 |

**Total estimated effort**: ~23 days (single developer)

## Architecture Decisions

1. **Separate subdomain**: User-facing app lives at `dash.tamma.dev`; admin stays at `app.tamma.dev`
2. **Shared JWT**: Both apps share the `tamma_session` cookie on `.tamma.dev` domain; JWT payload gains `orgId` claim
...
5. **Organization = tenant**: All resources (installations, workflows, settings) are scoped to an organization
6. **User model extended**: The `User` interface gains optional `passwordHash`, `emailVerified`, `orgId`, `authMethod` fields
