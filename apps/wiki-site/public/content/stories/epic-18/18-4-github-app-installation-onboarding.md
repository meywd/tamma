---
title: "Story 18-4: GitHub App Installation Onboarding"
sidebar:
  order: 180
---
...
2. **Install GitHub App** endpoint `GET /api/v1/onboarding/install-github` redirects to `https://github.com/apps/tamma-dev/installations/new` with `state` parameter encoding `orgId`
...
4. **Repository selection** endpoint `GET /api/v1/orgs/:orgId/repos` lists repositories available through the installation; `POST /api/v1/orgs/:orgId/repos/activate` activates selected repos
5. **Installation settings** endpoint `GET/PUT /api/v1/orgs/:orgId/installation` returns and updates installation-level settings (default branch, auto-run, etc.)
6. **Guided first run** endpoint `POST /api/v1/orgs/:orgId/repos/:repoId/first-run` triggers a test workflow on a selected repo to validate the setup
...
- [ ] Task 1: Implement onboarding status tracking
  - [ ] Subtask 1.1: Create `OnboardingStatus` interface: `{ emailVerified, hasOrg, orgId, hasInstallation, installationCount, hasActiveRepo, repoCount, hasFirstRun }`
...
- [ ] Task 2: Implement GitHub App installation redirect
  ...
  - [ ] Subtask 2.2: Generate `state` parameter: JWT-encoded `{ orgId, userId, nonce }` with 10-minute expiry
  ...
  - [ ] Subtask 2.5: On callback, validate `state`, link installation to org via `orgId` from state
...
- [ ] Task 3: Update webhook handler for org-scoped installations
  ...
  - [ ] Subtask 3.2: When `installation.created` webhook arrives with `state` parameter, extract `orgId` and link
...
- [ ] Task 4: Implement repository selection
  - [ ] Subtask 4.1: Implement `GET /api/v1/orgs/:orgId/repos` — list all repos across org's installations (from stored repo data + live GitHub API fallback)
  - [ ] Subtask 4.2: Implement `POST /api/v1/orgs/:orgId/repos/activate` — accepts `{ repoIds: number[] }`, marks repos as active
  - [ ] Subtask 4.3: Implement `POST /api/v1/orgs/:orgId/repos/deactivate` — deactivates repos
...
- [ ] Task 5: Implement installation settings
  ...
  - [ ] Subtask 5.2: Implement `GET /api/v1/orgs/:orgId/installation/:installationId` — return installation details + settings
  - [ ] Subtask 5.3: Implement `PUT /api/v1/orgs/:orgId/installation/:installationId/settings` — update settings (admin+)
  - [ ] Subtask 5.4: Write tests

- [ ] Task 6: Implement guided first run
  - [ ] Subtask 6.1: Implement `POST /api/v1/orgs/:orgId/repos/:repoId/first-run`
...
| File | Change |
|------|--------|
| `packages/api/src/persistence/installation-store.ts` | Add `orgId` field to `GitHubInstallation`, add `listByOrgId()` method |
...
GET /api/v1/onboarding/install-github
    |  (generates state JWT with orgId, userId)
    v
...
User selects org/repos on GitHub, clicks "Install"
    ...
    +---> Redirect: GET /api/v1/onboarding/install-github/callback?installation_id=123&state=<jwt>
            |
            v
          Validate state JWT, link installation to orgId
            |
            v
...
The current `GitHubInstallation` model has no `orgId`. This story adds it:

```sql
ALTER TABLE github_installations ADD COLUMN org_id UUID REFERENCES organizations(id);
CREATE INDEX idx_installations_org_id ON github_installations(org_id);
```

Existing installations (created before orgs existed) will have `org_id = NULL` and continue to work for the admin dashboard. New installations created through the onboarding flow will always have an `org_id`.

## Implementation Notes

- GitHub App installations can be initiated two ways: (1) from Tamma's onboarding flow (has `state` with `orgId`), or (2) from GitHub Marketplace / the app page directly (no `state`). For case (2), the installation is stored but not linked to an org until the user claims it via a "link installation" UI.
...
- **18-3**: Organization model and membership
