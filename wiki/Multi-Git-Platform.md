...
**Status**: COMPLETE (2026-08-09) — all phases P0–P6 of `docs/stories/epic-31/EXECUTION-PLAN.md` executed. Every production plane (git mediation, CI, agent dispatch, engine callbacks, webhooks, CI secrets) resolves through the driver abstraction; GitHub, Gitea and GitLab drivers are fully real (Forgejo rides the Gitea shim); the acceptance headline — one cycle's git surface completing on Gitea with zero GitHub configuration — is green in the nightly full-stack E2E. 10 core stories done + 2 deferred (Bitbucket, Azure DevOps).
...
**GitHub, Gitea and GitLab all implement these** (Forgejo through the Gitea shim); the two deferred drivers (Bitbucket, Azure DevOps) answer with `capability_unsupported`. Support is version-gated per installation — the factory probes the instance version at driver construction and narrows `PrLifecycle` away below the floor (Gitea 1.14, GitLab 13.9) or when the probe fails, and a contract test pins that a driver's advertised capabilities and its verbs' actual behavior can never disagree. The platform mechanics differ under one surface: GitHub toggles draft via GraphQL, Gitea's draft IS the `WIP:` title prefix, GitLab's the `Draft:` prefix; Gitea labels resolve names→ids, GitLab resolves reviewer usernames→ids inside the driver. Where support is genuinely absent, workflows do not fail: every capability-gated action is preceded by an is-supported **check step** and paired with a defined **alternative step** (EXECUTION-PLAN §4) — un-draft unsupported marks the step satisfied with an audit event, an unresolvable reviewer skips-with-label, a rejected line-anchor downgrades to a plain `file:line` comment, an unsupported merge method falls back rebase→squash→merge consuming exactly the typed `merge_method_unsupported` code, and a platform with no CI dispatch routes to the human merge gate. Every degradation emits a DCB audit event; silent skips are forbidden.
