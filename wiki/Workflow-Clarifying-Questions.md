...
> **Epic 39 (Story 39-13) — now a `document-lifecycle` binding (produces `Clarification`).** This workflow is a thin binding over the generic [Document Lifecycle](Document-Lifecycle) (`produce → validate → review → revise → accept`), run **twice** over the one `Clarification` document: run A produces the questions with the `(product_owner, clarify-requirements)` cell, the workflow delivers them and suspends on the human-answer bookmark, then run B produces the resolution with the `(product_owner, incorporate-answers)` cell. The old bespoke per-phase parsers (`ClarifyParsing`) and error-`Finish` terminals are **deleted**; each phase's validation, review, revision, and typed escalation are owned by the lifecycle. The bookmark-suspend/secure-resume seam is unchanged. The `CLARIFY.*` events still emit, now **alongside** the generic `DOCUMENT.*` events. The Flow Diagram and "Fail-Closed Parsing" section below describe the retired bespoke flow, kept for historical reference.
