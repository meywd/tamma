...
**Category**: Foundation
**Status**: DONE — see audit `docs/superpowers/plans/2026-05-29-epic-28-status-audit.md`. **AC3 release-build hard-fail closed by 2026-05-30 follow-up, REVISED 2026-05-31** (residual #2 of the 2026-05-30 Epic 28 residual-verification report). The guard `DependencyInjection.GuardTenantIsolationInProduction(isProduction, requireTenantIsolation, controlPlaneConnectionString)` (in `apps/tamma-elsa/src/Tamma.Data/DependencyInjection.cs`) throws `InvalidOperationException` only when `IsProduction() && Tamma:RequireTenantIsolation==true && CP-string is null/whitespace`; no-op otherwise. **2026-05-31 revision:** the first cut fired on ANY Production host without a CP string, which crash-looped the Deploy-to-VPS job — shared-infrastructure mode (every tenant on the central Postgres via Phase-3 RLS, `ConnectionStrings:ControlPlane` deliberately unset) is the documented production DEFAULT that the Hetzner VPS runs (`docker-compose.prod.yml`), not a misconfiguration. The guard now requires the explicit `Tamma:RequireTenantIsolation=true` opt-in, so it protects an operator who *intends* per-tenant DBs but forgets the CP string, without bricking the supported shared-DB topology. Wired in `Tamma.Api/Program.cs` after `AddTammaData`, before the CP-string pool gating; the flag is read via `Tamma:RequireTenantIsolation` (default false) and documented explicit-false in `docker-compose.prod.yml`. Tests: `apps/tamma-elsa/tests/Tamma.Api.Tests/Epic28/StubResolverProductionGuardTests.cs` (6 tests incl. the shared-DB-default-in-Production case).
