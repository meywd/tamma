...
1. **Entities and DbSets.** `WorkItemEntity`, `ProjectEntity`, `IterationEntity` and `TrackerPreference` in `apps/tamma-elsa/src/Tamma.Data/Entities/`, with `DbSet`s on `TenantDbContext` and EF configuration in `TammaModelConfiguration.cs`. `WorkItemEntity` carries at minimum: `Id` (UUIDv7), `ProjectId`, `Key` (the `WorkItemRef.ToWire()` string), `Number`, `Kind`, `Status`, `Priority`, `IssueType`, `Title`, `Description`, `ParentId`, `IterationId`, `Rank`, `AssigneeUserId`, `CreatedByUserId`, `EstimateHours`, `ExternalRefJson`, `CreatedAt`, `UpdatedAt`, `ClosedAt`, `Version`.

2. **Tenant migration `AddTrackerCore`** creating `projects`, `work_items`, `iterations`, `tracker_preferences` in the tenant schema.

3. **Constraints mirror the Core vocabulary.** `ck_work_items_status` and `ck_work_items_kind` enumerate exactly the wire strings of 44-0's `WorkItemStatus` and `WorkItemKind`; a test asserts constraint and enum agree member-for-member. `ck_tracker_preferences_principal_xor` uses the **strong** XOR form.
