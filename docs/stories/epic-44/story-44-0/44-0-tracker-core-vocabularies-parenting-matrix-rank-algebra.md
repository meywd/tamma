...
- **The depth-4 chain that already exists in code.** `DecompositionTask` (`Tamma.Core/Documents/Types/Decomposition.cs:29`) and `PlanTask` (`Types/Plan.cs:12`) are separate shipped record types, so `Epic → Story → DecompositionTask → PlanTask` is **depth 4**. The epic README lists materializing `PlanTask`s as a named v2 candidate. A `MaxDepth = 3` forecloses the epic's own roadmap before it is written; AC4 raises it to 6.
- **Both of those types already carry `dependsOn`** (`Decomposition.cs:36`, `Plan.cs:17`). Dependency between units of work is an *existing* concept in this codebase and it is not parenting. AC14 gives it a vocabulary rather than letting it be smuggled into the tree.
...
1. **`WorkItemKind`** — a `[Wire]` enum in `apps/tamma-elsa/src/Tamma.Core/Tracking/WorkItemKind.cs` with exactly four members: `epic`, `story`, `task`, `spike`. Wire round-trip and count (`Be(4)`) pinned by tests.
   **`bug` and `chore` are deliberately absent.** `TriageIssueType` (`TriageDecision.cs:23-31`) already carries both, and AC12 binds it as the work item's *type* axis — so `Kind=Bug` would be a second, partially-overlapping vocabulary for the same fact, and `(Kind=Bug, Type=Feature)` would be a representable statement that means nothing. Kind answers *what may contain what*; type answers *what sort of thing is it*. Two axes, two vocabularies, no overlap.
...
4. **`TrackerHierarchy` — structural invariants, not a `(parentKind, childKind)` matrix.** The type exposes:
   ...
   - No cycles and depth ≤ `MaxDepth` are stated here as the remaining invariants; both need a parent chain and therefore I/O, so 44-3 enforces them against these constants.
...
13. **`EstimateScale`** — a `[Wire]` enum with exactly five members: `not_used`, `linear`, `fibonacci`, `exponential`, `t_shirt`. Count pinned at 5. It is **project configuration** (stored on `Project` by 44-1); the work item stores a scale-free `Estimate` (`decimal?`), not `EstimateHours`.
    **Why the rename:** an `EstimateHours` column names the scale in the column, which means changing scale is a migration and mixing scales across projects is impossible. Every scale Linear ships (`notUsed, exponential, fibonacci, linear, tShirt`) is pointedly *not* hours — because an hours-shaped estimate invites the reading that the number is a commitment. `Estimate` + a project-level scale is one nullable number and one enum, and it is the shape a team can actually change its mind about.
