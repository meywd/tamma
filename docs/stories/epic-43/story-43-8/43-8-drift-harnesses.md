...
1. **Binding surface: one metadata type, two authoring shapes.**
   ...
   - `RouteHandlerBuilder.Governs(ActionKey)` — an extension in `Tamma.Api/Infrastructure/GovernsExtensions.cs` that adds `ActionGateMetadata` and **nothing else**. ~~and (Story 43-9) the enforcement filter~~ — **SUPERSEDED 2026-08-01 by Story 43-9 Decision D15**: `.Governs()` stays metadata-only for ever, and enforcement is a separate per-route opt-in (`.EnforcesGovernance()` / `[EnforcesGovernance]`). Three reasons, recorded on `IGovernanceEnforcementMetadata`: (1) one line inside `Governs()` would have flipped 17 routes into live 409 gates at once with no per-route review; (2) `POST /api/v1/llm/call` must be incapable of blocking in every version (epic D2), and the only way to make that structural rather than a carve-out keyed on an action name is that it never writes the second line; (3) **an `IEndpointFilter` does not run for an MVC endpoint**, so a filter attached here would have enforced 17 routes and silently skipped `MentorshipController`'s 4 while reading as "all bindings are enforced". Used by minimal APIs.
   ...
   Both produce the *same* metadata object, so every harness and the Story 43-9 filter read exactly one thing. **(2026-08-01, D15: the ENFORCEMENT opt-in is a second metadata type, `IGovernanceEnforcementMetadata`, with its own two authoring shapes for the same reason.)**
