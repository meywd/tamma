...
- **⛔ Story 43-5 follow-up F11 — a break-glass override for the FAIL-CLOSED posture. BLOCKING, and unbuilt.** Recorded 2026-07-30; the full write-up (proof, what already works, and the product questions to answer) is under "Open follow-ups" in the 43-5 story. Summary: since the 43-5 F6 close, a non-authoritative governance snapshot makes the gate DENY, and there is **no config flag, env var or admin endpoint** that can force it open. Today the blast radius is one seam (the tool loop) — every catalogued tool denied, every agent loop inert, in both SaaS and single-user-with-a-control-plane deployments. It self-heals within the 60 s refresh TTL and logs loudly at ERROR, so this is a *recovery-lever* gap, not a diagnosis gap. **This story carries that posture into four more seams with larger blast radius**, at which point a control-plane outage stops an agent from doing anything at all with no supported recovery short of a code change. Decide the knob's shape (who may set it, time-bounded or sticky, how each use is audited, wholesale or per-risk-tier) and land it **before** the seams go live. Whatever it is, it must be loudly logged at every use.
