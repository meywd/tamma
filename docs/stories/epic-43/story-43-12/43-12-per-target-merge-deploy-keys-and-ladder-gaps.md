...
| 2026-08-03 | 1.1.0   | Implemented: 10 descriptors minted at zone levels; coarse git.pull-request.merge + deploy.promote-prod retired; merge route resolves per-target key from PR base branch (multi-binding + MergeTargetActionKeySelector, fail-closed to git.merge.main); Seam E rebound to effect:deploy.prod; POST /api/engine/command deleted; agent-action:deploy/rollback pinned 90/95; all count pins moved (197->205, effect 39->47, dial 155->163, ungoverned 216->215, in-scope 239->238, withSites 21->23, mediation-client 17->19). DEVIATION: AC4's QA/UAT stage-entry gate calls on deploy.qa/deploy.uat (and the WaitForDeploymentApprovalActivity Stage input + resume plumbing) were DEFERRED — deploy.qa/deploy.uat are catalogued at their levels but not yet wired as live workflow gates (the resume plumbing crosses AdlEndpoints/IElsaWorkflowService, outside this story's fence, and is a large behaviour change flagged as risky). deploy.prod is live via Seam E. | Claude |
