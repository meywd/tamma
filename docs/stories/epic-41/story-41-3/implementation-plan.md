...
- **The gates this story must move:** `WorkflowInterfaceGraphTests.cs:45` `HaveCount(16)` + the `reconciled` array `:102-123`; `ContractBindingTests.cs:82` `Bindings`, `:681` classify-or-fail; `TaxonomyDriftBuildTests.cs:125` `ExpectedContributingWorkflows`, `:460` `ScanLifecycleBindingDispatches`; `ResumableStandardStructuralTests.cs:108/:159/:238/:266`
...
6. **Rule-1 clause (f) is a two-edit lockstep and the epic README names only one.** Besides
   ...
   `Seeded_declarations_are_provisional_except_reconciled_bindings` (`:96`) asserts **bidirectionally**
   against a hardcoded `reconciled` array (`:102-123`): every listed id must be `!Provisional` and every
...
- **D2 — Set-scoped lineage anchor: `backlog:{repository}:{backlogScope}`, folded through
  `CreationBindingHelper.ScopeIssueId`'s normalisation.** The store's only read key is `issueId` (Correction
...
- **D3 — Ranking evidence is gathered by N bounded per-item reads behind the `FreshRun` gate, and its
  ...
  `(item.issueId, "triage-decision")` and one for `(item.issueId, "findings")`, inside an Elsa `ForEach`
...
- **D8 — Acceptance posture is 41-1b's arm, asserted here.** `AcceptanceDefaults.For` ends in `_ => Rules`
  ...
  ordering. The required row is a **`product_owner` single reviewer** with the acceptor per autonomy, and
...
3. **CREATE `apps/tamma-elsa/src/Tamma.ElsaServer/Workflows/Helpers/BacklogBindingHelper.cs`** — pure,
   Elsa-free, total, fail-closed: `BuildAnchor(repository, backlogScope)` (D2 — deterministic, normalised
   through the same segment transform `ScopeIssueId` uses), `ParseItems(itemsJson)` → a bounded list (D3's
...
5. **MODIFY `apps/tamma-elsa/src/Tamma.Core/Documents/DocumentTypeRegistry.cs`** (`BuildSeed`) — add
   ...
   **MODIFY `apps/tamma-elsa/tests/Tamma.Core.Tests/Documents/WorkflowInterfaceGraphTests.cs`** — `:45`
   `HaveCount(16)` → `HaveCount(17)` (or +1 on whatever 41-2/41-6 left it at) with the reason in the
   comment, **and** add `"backlog-prioritization"` to the `reconciled` array `:102-123` (Correction 6).
...
7. **MODIFY the drift gates.** `ContractBindingTests.cs`: add
   `[("product_owner","prioritize-backlog")] = new("BacklogOrderingDocumentType.Validate", [ … 41-1b token
   groups … ])` with a comment naming `Tamma.Core/Documents/Types/BacklogOrdering.cs` as the shape
   authority. `TaxonomyDriftBuildTests.cs`: add `"BacklogPrioritizationWorkflow"` to
   `ExpectedContributingWorkflows` (`:125`). Verify (do not pre-edit) `MinExpectedDispatchPairs` (`:110`, a
   floor) and `EveryConcreteWorkflow_IsIntrospectableOrAllowListed` (`:397`).
...
- **`BacklogPrioritizationLifecycleExecutionTests`** (Testcontainers) —
  (a) **happy path with evidence:** two backlog items, one with an accepted `triage-decision` and one
  without → scripted valid draft → review approve → `Accept` resume → `status=completed`; asserts the
  evidence blob reached the producer variables and the item with no triage still appears in the ordering
  (D3's graceful degradation, Correction 4). **Covers AC1, AC2.**
...
- **Drift gates (self-verifying, steps 5/7)** — `ContractBindingTests`, `TaxonomyDriftBuildTests`,
  `WorkflowInterfaceGraphTests` (count **and** `reconciled` bidirectional) and
...
| 2 — consumes upstream `TriageDecision`/`Findings` as evidence | 4 (D3) | ExecutionTests (a); HelperTests accumulation (degradation caveat in Correction 4) |
...
- **Blocked by:** **41-1b** — hard. `DocumentTypeKey` has exactly 10 members today (verified) and
  ...
  the **human path too**. Also **Epic 39** (39-6/39-7/39-8/39-10/39-11 — all landed and verified).
- **Soft-blocked by 41-2** — only for D7's shared `EmitDomainLifecycleEventActivity`. A local copy unblocks
  it at the cost of a later mechanical merge.
