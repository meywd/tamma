...
1. **Agent host — one orchestrator PER TENANT.** A hosted long-running service (its own process or a hosted service in the engine deployment — decided and documented here) runs orchestrator agents at tenant grain: in SaaS mode **each tenant has its own agent instance** — its own persistent session, its own context, its own channel binding — never a shared agent multiplexing tenants; in single-user mode there is exactly one, the sole user's. A tenant's agent's tools are hard-scoped to that tenant's data (the per-tenant connection/authorization seam — a cross-tenant read is impossible at the tool layer, not merely unprompted), and its context never contains another tenant's material. The host manages per-tenant lifecycle (start on demand, idle without token burn, stop/evict) and per-tenant spend guardrails. Each agent starts, reconnects, and resumes consuming after crashes; a restart re-hydrates working context from the event store/document store and drains that tenant's persisted request outbox — no request is lost or double-answered (idempotent decision delivery via the 39-8 resume discipline).

2. **The toolset — closed, typed, audited.** The agent's tools are a closed registry (drift-tested like the taxonomy), covering at minimum: **git** (read: log/diff/blame/file content for the worked repos); **event store** (DCB queries by tags/issueId — the Story 4-7 query surface); **logging** (structured log search); **workflows** (list/inspect instances, dispatch, resume, cancel via the Elsa management APIs); **documents** (39-11 store + lineage reads); **knowledge** (`kb_search`/`rag_query` over the 39-21 tenant-isolated RAG pipeline); **acceptance rules** (`get_acceptance_rules` over 39-5's `IAcceptanceRulesResolver`); **issue/platform reads**. Every tool invocation emits an `ORCHESTRATOR.TOOL_INVOKED` DCB event (tool name, args digest, correlation) so the agent's reasoning trail is reconstructable. Write-capable tools (dispatch/resume/cancel) are individually allowlisted and RBAC-scoped; everything else is read-only.

3. **Acceptance + routing duty — the agent is the single routing point.** The agent consumes EVERY `AcceptanceRequest` from the workflow channel, reads the effective acceptance rules + **autonomy level (70–100)** via its tool (and receives the resolved payload embedded in the request for audit pinning), inspects the document + `Review` + lineage, and makes the ROUTING decision first: **decide itself** (the more so the higher the autonomy level) or **assign the decision to a role** — addressing a tenant role whose 39-20 audience (role-holders ∩ visibility rule: initiator or repo access) is server-validated non-empty (an unknown role or empty audience is refused and the agent re-routes), which creates a 39-19 task visible to every eligible role-holder — completable from the Task View or by asking the orchestrator in chat, first authorized completion wins. A self-decision is a typed `AcceptanceDecision` (`Accept | RequestRevision(notes) | Escalate(reason)`) through the 39-8 resume path; an assignment defers to the human's decision through the same path. The 39-5 guardrail function wraps whichever answer arrives — a blocking review cannot be accepted, round bounds cannot be exceeded — and a test pins that. Self-decisions emit `APPROVAL.PROVIDED` with `channel=orchestrator` + the rules version; assignments emit the 39-20 `TASK.ASSIGNED` with the eligibility basis and autonomy context. At autonomy 70 the agent assigns nearly everything; at 100 it decides everything the rules allow — same code path, different dial reading.
...
6. **Per-mode ownership.** The agent's identity, provider binding, and configuration answer the CLAUDE.md two-scoping-models rule: single-user mode — one agent owned by the sole user; SaaS — **per-tenant** agent identity and configuration (which provider/model, context budget, spend limits), tenant-owned, admin-editable. The full per-tenant triad is: the tenant's **automation config** (39-5 rules + autonomy level, keyed `tenant_id`), the tenant's **orchestrator process** (this story), and the tenant's **channels** (39-18) — one boundary, drawn once. Written down before code. Isolation tests drive two tenants' agents concurrently and assert no cross-tenant tool reach, context bleed, or channel delivery.
...
- **Prerequisite:** 39-5 (acceptor contract + rules resolver + autonomy level + guardrails), 39-18 (the channels it consumes — lockstep), 39-8 (resume path + APPROVAL/ESCALATION events), 39-11 (document/lineage reads), 39-20 (role-audience resolver + task events — lockstep), Story 4-7 event query surface.
...
| 2026-07-21 | 1.2.0   | Per-tenant boundary made explicit: one orchestrator agent instance per tenant (own session/context/channel binding, hard tool scoping, per-tenant lifecycle + spend), forming the tenant triad with the automation config (39-5) and channels (39-18) | Claude |
