...
- **assign to a role** — address the decision to a tenant role (server-validated against the 39-20 audience: role-holders within the tenant, intersected with the visibility rule — initiator or repo access). The task lands in the 39-19 Task View of EVERY eligible role-holder, and can equally be handled conversationally by asking the orchestrator in chat; whichever surface, the human's decision comes back through the same 39-8 resume surface, and the first authorized completion wins (idempotent — everyone else's inbox clears).
...
- 39-20 access model — computes the role audience an assignment reaches (role-holders within the tenant ∩ visibility rule; teams/roles/permissions respected)
