...
- `apps/tamma-elsa/src/Tamma.ElsaServer/Workflows/LlmCallWorkflow.cs` — `DefinitionId = "llm-call"`, inputs `agentRole` (the real dispatch input key — NOT `role`, which is only a legacy fallback)/`action`/`variables`/`tenantId`/`enableTools`, outputs `success`/`llmResponse`/`workflowOutput`
...
- **D7 — Exit contract: helper-built `DocumentLifecycleResult`, no bare fault path; `Rejected` is a first-class terminal distinct from `Accepted`.** New record in `Tamma.Core/Documents/DocumentLifecycleResult.cs` (owned by this story per story AC3 "outcome enum is owned by [39-2]; the lineage-carrying result is 39-6's"): `Status` (`accepted | rejected | escalated` — three terminals), `Outcome` (`DocumentLifecycleOutcome?`, **null on BOTH `accepted` and `rejected`; non-null ONLY on `escalated`**), `DocumentId`, and a `DocumentLineage` record (every draft envelope id + state, every `Review` id, rounds used, repair attempts used, last domain-phrased violations, effective-rules reference). **Because `Outcome` is null on both accept and reject, a parent CANNOT distinguish accepted from rejected by the outcome enum — parents MUST switch on `Status` FIRST (`accepted | rejected | escalated`), and only the `escalated` branch reads `Outcome`.** A human `Reject` (decider channel `user`/`api`) terminates the lifecycle in `Rejected` (a settled decision kind, `DocumentLifecycleHelper.BuildRejected`, envelope → `Rejected` state) — it is NOT an escalation and NOT an accept. Every stage-failure edge (llm-call `success=false`, unparseable payload, validator exhaustion, review failure) routes through fail-closed `FlowDecision`s into `DocumentLifecycleHelper.BuildOutcome(...)` — the `IssueDecompositionWorkflow.ReadSuccessFlag` posture generalized. Mapping: PRODUCE/repair `llm-call` failure or invalid payload consumes a repair attempt, exhaustion → `ValidationExhausted`; REVIEW producer failure or an unusable `Review` → `ReviewUndecidable`; revise rounds exhausted → `RoundsExhausted`; ambiguity score over the rules threshold → `AmbiguityAboveThreshold` (D8). All four exit `Escalated` envelope state (39-2 D4). Outputs: `status`, `outcome`, `documentId`, `lifecycleResult` (full JSON) — parents branch on `status` first, then (only for `escalated`) `outcome`.
...
6. **CREATE `apps/tamma-elsa/src/Tamma.ElsaServer/Workflows/DocumentLifecycleWorkflow.cs`** — house `WorkflowBase`/Flowchart style (copy skeleton from `IssueDecompositionWorkflow`, loop wiring from `PlanReviewWorkflow`): `builder.DefinitionId = "document-lifecycle"; builder.Version = WorkflowVersions.ComputedVersion;`. Graph: `Init` (read+validate inputs per D2/D4, mint sessionId, seed `LifecycleState`) → `DispatchProduce` (`DispatchWorkflow("llm-call")` with producer spec — the dispatch Input dictionary keys the role as **`agentRole`** (the real llm-call input key, not `role`) plus `action`/`variables`, AND threads `documentType` and `issueId` onto the wire request so 39-9's inner repair ring — which selects the validator from `documentType` on the wire request — is reachable through the generic lifecycle; `WaitForCompletion=true`) → `EmitProduced` (SUCCESS/FAILED) → `ValidateDraft` (SetVariable lambda delegating to `DocumentTypeRegistry.Resolve(typeKey).Validate(payload)` + `EmitValidated`) → `RepairCheck` (`FlowDecision` on `ShouldRepair`; True → `DispatchRepair` llm-call with `BuildRepairVariables` → back to `ValidateDraft`; False+invalid → `BuildOutcome(ValidationExhausted)`). **This graph-level `DispatchRepair` is the OUTER repair ring — a fresh-conversation llm-call bounded by `MaxValidationRepairAttempts`. The cheap INNER ring (same-conversation validator→feedback→repair, the README's "innermost ring") is 39-9's managed-layer mechanism, parked default-OFF until a provider is configured; it runs inside a single produce dispatch and is invisible to this graph except through the wire result (see 39-9 D7).** → `AmbiguityCheck` (D8; over → `Escalated` path) → `EmitReviewRequested` → `DispatchReview` (D10) → `EmitReviewed` → `ReviseCheck` (`ReviewDecision.Approve` → ACCEPT; `RequestChanges`/`NeedsDiscussion` + `ShouldRevise` → `EmitRevisionStarted` → `DispatchRevise` (llm-call, `BuildRevisionVariables`, new superseding envelope) → back to `ValidateDraft`; exhausted → `BuildOutcome(RoundsExhausted)`; unusable review → `BuildOutcome(ReviewUndecidable)`) → **ACCEPT**: `BuildAcceptanceRequest` → `PublishAcceptanceRequestActivity` → `WaitForDocumentDecisionActivity` (39-8 gate; sessionId + tenantId inputs) → `ApplyGuardrails` (39-5 guardrail function in a SetVariable lambda) → route `Accept` → `EmitAccepted` + envelope→`Accepted` (`BuildAccepted`, `Status=accepted`, `Outcome=null`); `RequestRevision(notes)` → revise loop (counts a round; guardrail already converts over-budget to Escalate); `Reject(reason)` (human-only — the guardrail has already clamped any orchestrator-channel Reject to Escalate, D5/39-5) → `EmitRejected` + envelope→`Rejected` (`BuildRejected`, `Status=rejected`, `Outcome=null` — a first-class terminal distinct from Accepted, D7); `Escalate(reason)` → `EmitEscalated` + `BuildOutcome` (`Status=escalated`, `Outcome` non-null). Terminal `SetOutputs` (`status`/`outcome`/`documentId`/`lifecycleResult`) → `Finish` — one output shape on every path, parents switch on `status` first, per D7.
