...
- **D6 — Channel publish behind `IAcceptanceRequestPublisher`, stubbed until 39-18.** New interface (in `Tamma.Activities/Documents/`) with `Task PublishAsync(AcceptanceRequestWire request, CancellationToken ct)`; the activity resolves it via `context.GetService<T>()` (the `EventPersistenceMiddleware` service-resolution pattern — no captive dependency). This story registers a `LoggingAcceptanceRequestPublisher` (logs + no-op; the suspended gate still waits, matching 39-18's "no orchestrator connected ⇒ request waits, never defaulted"). 39-18 swaps in the outbox+SignalR implementation behind the same interface; tests use a capturing fake. If 39-18's `AcceptanceRequest` message record in `Tamma.Core` has landed first, use it; otherwise ship `AcceptanceRequestWire` here with the canonical field set and let 39-18 adopt it (coordinate — one record, one name, in `Tamma.Core`).
...
- **D9 — Revision mints a NEW envelope; state transitions only via `DocumentStateMachine`.** Draft → `Validated` on validator pass; `Validated → Reviewed` when the `Review` lands; `Reviewed → Accepted/Rejected` on decision; any typed outcome → `Escalated`. A revise turn creates a fresh `DocumentEnvelope.CreateDraft(..., supersedesDocumentId: prior.Id)` — never rewinds (39-2 D4). All transitions go through `envelope.WithState(...)` so an illegal transition throws `DOCUMENT.STATE.ILLEGAL_TRANSITION` (AC6). Envelopes live in the `LifecycleState` JSON; durable document-store persistence is 39-11 (events + workflow state suffice for 39-10 re-entry).
