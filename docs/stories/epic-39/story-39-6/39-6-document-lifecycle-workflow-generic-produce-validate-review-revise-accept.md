...
2. **The five stages per the README diagram.** PRODUCE (dispatch `llm-call` with the producer spec) → VALIDATE (the type's `Validate`; on violations, a **bounded repair turn** feeding domain-phrased violations back via a fresh `llm-call` — the **outer** repair ring; the cheap same-conversation **inner** ring (the README's "innermost ring") is 39-9's managed-layer mechanism, parked default-OFF until a provider is configured, but the bounded outer retry seam exists here) → REVIEW (dispatch the 39-7 producer, yielding a validated `Review` document) → on concerns, REVISE (bounded rounds: re-dispatch the producer with the `Review`'s notes — summary + issues with suggested fixes — included in `variables`) → ACCEPT (**always submits to the orchestrator**: build the `AcceptanceRequest` — document + `Review` + lineage + resolved rules incl. autonomy level — publish it on the workflow↔orchestrator channel (39-18), and suspend on the 39-8 gate until the `AcceptanceDecision` resumes it. The orchestrator (39-17) decides itself or assigns the decision to a 39-20-eligible user per the autonomy level — the lifecycle neither knows nor cares which: never an if-else that skips the decision, never an embedded accept-decision `llm-call`, and the 39-5 guardrail function wraps whichever actor answers).

3. **Exit contract: three terminal statuses (`accepted | rejected | escalated`), each done or a typed unhandleable outcome, nothing else.** The workflow's result carries a `Status` — `accepted` (published + `Accept` decision), `rejected` (a human `Reject` decision — a first-class terminal, NOT an escalation), or `escalated` (one of the closed outcomes `ReviewUndecidable`, `AmbiguityAboveThreshold`, `RoundsExhausted`, `ValidationExhausted`, enum in `Tamma.Core/Documents`) — plus a nullable `Outcome` that is **null on both `accepted` and `rejected` and non-null only on `escalated`**. Every terminal carries the full document lineage (envelope ids of every draft, every `Review`, rounds used, last violations). No bare exception path: an unexpected internal failure surfaces as a typed `escalated` outcome with lineage, never a silent workflow fault. **Because `Outcome` is null on both accepted and rejected, parent workflows MUST switch on `Status` FIRST (accepted | rejected | escalated) — the outcome enum alone cannot distinguish accepted from rejected — and read `Outcome` only on the `escalated` branch.** A drift test pins both the status set and the outcome set.
