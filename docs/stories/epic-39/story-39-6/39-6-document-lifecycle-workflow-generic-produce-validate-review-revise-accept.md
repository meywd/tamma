...
**Consumed contracts:** 39-2 envelope/state machine + registry, 39-3/39-4 validators, 39-5 acceptor contract (`AcceptanceDecision`, `IAcceptanceRulesResolver`, guardrail function, the `decide-acceptance` cell), 39-7 review producers (single/panel — dispatched by definition id from the review step), 39-8 suspend/escalation surface (the human-acceptor gate and the unhandleable-outcome sink).
...
8. **Integration test, one full cycle.** An NUnit integration test (Testcontainers Postgres, stubbed `llm-call` responses in the established test style) drives one type (e.g. `Decomposition`) through: invalid draft → repair → valid → review with concerns → revise → approve → full-auto: the stubbed orchestrator-acceptor decision turn returns `Accept` (asserting the acceptor was actually dispatched with the resolved rules in its variables, not short-circuited); and a second path to each unhandleable outcome class — asserting emitted events, final envelope state, and the outcome payload's lineage completeness.
...
- The human-acceptor path (supervised mode) should register its bookmark via the 39-8 generalized gate (bookmark name folding tenant + a session/document id, per the `DesignResumeEndpoint.cs` posture) rather than minting its own suspend convention. The orchestrator-acceptor path (full-auto) is an ordinary `llm-call` dispatch of the 39-5 `decide-acceptance` cell — it counts toward the mediation-invariant test like every other dispatch.
