...
I want a **single generic Elsa sub-workflow implementing `produce → validate → review → revise (bounded) → accept` for any registered document type — parameterized by a producer dispatch spec, a document type key, and an acceptance policy — that exits only on done or a typed unhandleable outcome, and emits a `DOCUMENT.*` DCB event on every transition**,
...
- The supervised ACCEPT GATE should register its bookmark via the 39-8 generalized gate (bookmark name folding tenant + a session/document id, per the `DesignResumeEndpoint.cs` posture) rather than minting its own suspend convention.
- `AmbiguityAboveThreshold` is policy-triggered (39-5 threshold vs. an `AmbiguityAssessment` score) — the lifecycle raises it when the produced/associated assessment crosses the configured threshold; it is listed here because the outcome enum is owned by this story.
...
- **Prerequisite:** 39-2 (envelope/state/registry/outcome home), 39-3 + 39-4 (validators and `Review`), 39-5 (policy + `Decide`).
- **Lockstep:** 39-7 (review producer contract), 39-8 (suspend/resume + escalation sink for the supervised gate and unhandleable outcomes).
