...
2. **DesignProposal on the generic accept gate.** `WaitForDesignApprovalActivity` and the bespoke approval branch are retired; design acceptance publishes to the orchestrator and suspends via 39-8's generic gate on the canonical tenant-folded bookmark (the orchestrator self-decides or assigns per the autonomy level — `Design` is expected to be pinned to human assignment by default). **`DesignResumeEndpoint` behavior is preserved through the generalized resume surface**: the existing route contract (payload shape, approve/reject semantics, serialization-tolerant read-back, tenant folding, response codes) keeps working — either as a thin forwarder onto the generic resume endpoint or via route aliasing — and the existing `DesignResume*` test suites pass unchanged or with mechanical-only updates.
