...
- **Workflow↔orchestrator channel** — engine/workflows on one side, the 39-17 agent on the other, **bound at tenant grain**: a tenant's workflows publish to that tenant's channel partition, consumed by that tenant's agent instance (never a shared agent across tenants); the outbox is per-tenant likewise. Engine-internal trust posture (the `Tamma.Api` → engine hop precedent used by the resume endpoints): the agent authenticates as its tenant's orchestrator principal; workflows publish via the engine.
...
1. **Typed message contracts in `Tamma.Core`.** A closed, drift-tested message set: `AcceptanceRequest` (document + `Review` + lineage refs + resolved rules payload incl. autonomy level + correlation/decision-session id), `AcceptanceDecision` (the 39-5 type + decider + rules version), `TaskAssigned` (the orchestrator's assignment notification: task ref + addressed role + resolved audience + eligibility basis + autonomy context — delivered to every eligible role-holder, not one user), `EscalationRaised` (the 39-8 lineage payload), `EscalationDisposition`, `GuidanceQuery`/`GuidanceReply`, `AgentConversationMessage`. No stringly-typed message bodies; serialization round-trip tests.
