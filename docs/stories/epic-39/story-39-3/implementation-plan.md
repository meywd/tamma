...
- **D7 — Round-trip suite needs the OLD parsers ⇒ `Tamma.Core.Tests` gains a `ProjectReference` to `Tamma.Activities`.** AC7 demands "the *old* parser still parses" and AC9 confines the diff to `Tamma.Core/Documents/**` + `Tamma.Core.Tests/**`. The csproj lives inside that surface; the reference direction (`Tamma.Core.Tests` → `Tamma.Activities` → `Tamma.Core`) has no cycle. Cost: Elsa 3.5.3 + Tamma.Data arrive transitively at compile time — the round-trip tests themselves stay pure in-memory (the parsers are static and context-free), no Docker. Tension noted: 39-2 kept `Tamma.Core.Tests` dependency-light; the story's AC9 diff surface wins here. If reviewers balk, the fallback is relaxing AC9 to admit one file in `Tamma.Activities.Tests` — an open question, not the plan of record.
...
2. **CREATE `apps/tamma-elsa/src/Tamma.Core/Documents/Types/Decomposition.cs`** — records mirror `DecompositionModels.cs` shapes (immutable records instead of mutable classes), plus the document type:
   ...
   `Validate` deserializes via `DocumentJson.Options` (a `JsonException` → a single `MALFORMED_PAYLOAD` violation, never a throw out of `Validate`), then runs the domain checks. Cycle check: iterative DFS with an explicit stack over the id→dependsOn adjacency; on a back-edge, walk the stack to render the cycle path (D10). Topological order asserted via Kahn's algorithm; when it cannot complete, emit `NO_PREREQUISITE_ORDER` alongside the cycle violation(s).
...
9. **MODIFY `apps/tamma-elsa/tests/Tamma.Core.Tests/Tamma.Core.Tests.csproj`** — add `<ProjectReference Include="..\..\src\Tamma.Activities\Tamma.Activities.csproj" />` (D7). Then **CREATE `apps/tamma-elsa/tests/Tamma.Core.Tests/Documents/Types/BaselineSubsumptionTests.cs`** and **`RoundTripCompatibilityTests.cs`** (fixtures copied verbatim from the three `*ParsingTests.cs` files, each with a comment naming its source constant — they cannot be referenced across test assemblies).
...
12. **Verify AC9 by construction**: `git status` shows only `Tamma.Core/Documents/**`, `Tamma.Core.Tests/**`, and this story's docs directory; `dotnet test apps/tamma-elsa/tests/Tamma.Core.Tests` and `apps/tamma-elsa/tests/Tamma.Activities.Tests` both green (the latter proves `ContractBindingTests` and the parser tests are undisturbed).
...
- **`Tamma.Core.Tests` → `Tamma.Activities` reference drags in Elsa/EF transitively.** Mitigation: compile-time only, tests stay in-memory; isolated to one csproj line that 39-13 can retire once the old parsers are deleted. Fallback documented in D7.
...
| 9 | Subsumption + round-trip suites (fixture porting, csproj ref) | 0.75 |
