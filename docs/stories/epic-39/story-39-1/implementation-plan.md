...
- **Enumeration cross-check (AC2/AC8 seeding):** the Step 2 dump ⊇ `TaxonomyDriftBuildTests.ExpectedContributingWorkflows` (20 workflows) and ≥ 40 pairs; any delta is investigated, not hand-waved. `dotnet test --filter TaxonomyDriftBuildTests` green confirms the enumeration itself is healthy before trusting its output.
