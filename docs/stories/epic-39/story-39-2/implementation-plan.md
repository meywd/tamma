...
**Posture for downstream stories (decision, honored by 39-3 D7 and 39-4 D8):** `Tamma.Core.Tests` stays **dependency-light and Docker-free** — it must NOT gain a `ProjectReference` to `Tamma.Activities`/`Tamma.ElsaServer` (which would drag Elsa 3.5.3 + Tamma.Data transitively into the Core test project). The cross-parser subsumption/round-trip tests that later stories need (which invoke the OLD `Tamma.Activities` parsers to prove "the old parser still parses") therefore live in **`Tamma.Activities.Tests`** (which already references both `Tamma.Activities` and `Tamma.Core`), NOT in `Tamma.Core.Tests`. Only pure, in-memory new-type validation tests belong in `Tamma.Core.Tests`.
