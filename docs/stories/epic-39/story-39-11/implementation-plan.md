...
- **Stubbed, not pulled in:** 39-8 (its escalation payload will serialize `IssueDocumentLineage` slices — nothing imported here); 39-21 (`DOCUMENT.ACCEPTED`-triggered indexing reads this store later); dashboards (consume the endpoints as-is).
