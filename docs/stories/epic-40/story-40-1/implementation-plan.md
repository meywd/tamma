...
- ~~`IGitHubActionsClient.cs:33` / `OctokitGitHubActionsClient.cs:63` / `NullGitHubActionsClient.cs:16`~~ — **these files no longer exist** (verified 2026-08-18; Epic 31 retired the GitHub-only seam). The presence check is an inline platform-abstraction read in the mediation service: `driver.Client.GetFileContentAsync(new GetFileContentRequest(owner, name, $".github/workflows/{workflowFile}", body.Ref))`, blocking only on a positive `NotFound`, with GitLab exempt. **Step 5's commit path must be re-planned:** `IGitPlatformClient` (`Tamma.Platforms.Abstractions/IGitPlatformClient.cs:47`) has a file **read** verb and no file **write** verb, so scaffolding a file into a user repo needs a new abstraction verb plus a per-driver implementation.
