...
| Question | Finding |
|---|---|
| **Does `tamma-agent.yml` exist anywhere?** | **It does now — 40-1 shipped it on 2026-08-18** (`apps/tamma-elsa/runner/github-actions/` canonical + `.github/workflows/tamma-agent.yml` as Tamma's own install, pinned to the dispatcher by `RunnerContractTests`). The finding below is the pre-40-1 state, kept because the rest of the epic was scoped against it. *Also corrected 2026-08-18:* `IGitHubActionsClient`/`OctokitGitHubActionsClient`/`CheckWorkflowFileAsync` **no longer exist** — Epic 31 retired that seam; the presence check is an inline `driver.Client.GetFileContentAsync` probe in `AgentDispatchMediationService`, which still owns the fail-loud message. **Historical:** No. Not in Tamma's own `.github/workflows/` (which has `tamma-worker.yml` but no `tamma-agent.yml`), not in any `templates/` dir (only `.dev/templates` exists, unrelated), no `run-claude-code.sh`/`collect-results.sh`. Story 19-1 defined the contract; the file was never authored. No scaffolding/generator installs it into a user repo. **Corrected (was: "`AgentDispatchMediationService.CheckWorkflowFileAsync` only checks presence")** — that member is not on the mediation service. The presence check is `IGitHubActionsClient.CheckWorkflowFileAsync` (`IGitHubActionsClient.cs:33`, implemented in `Tamma.Api/Services/GitHub/OctokitGitHubActionsClient.cs:63`); `AgentDispatchMediationService` *calls* it at `:101` and owns the **fail-loud policy + message** at `:107-112` (`WorkflowNotFound` + "Add the Tamma agent workflow template to .github/workflows/"). It is **assumed user-provided and undocumented** today. The filename itself is hardcoded as a default in **six** places (see 40-1) — including the engine→API wire default `AgentDispatchRunApiRequest.WorkflowFileName` (`TammaApiModels.cs:494`) — so making it configurable/versioned is a six-site change, not a one-site change. |
...
|-------|-------|----------|--------|-------------|
| 40-1 | The `tamma-agent.yml` Runner Contract & Repo Scaffolding (+ single-user CLI parity) | P0 | in-progress (runner + pins + AC8 shipped; AC7 server half open) | 6-8 days |
