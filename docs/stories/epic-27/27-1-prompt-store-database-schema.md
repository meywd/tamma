...
so that prompts are persisted in the database with account-level isolation and can be resolved using a two-tier fallback (account override then system default).

## Acceptance Criteria

1. A `prompts` table exists with columns: `id` (UUID PK), `account_id` (UUID nullable, FK to tenants), `role` (TEXT NOT NULL), `action` (TEXT NOT NULL), `template` (TEXT NOT NULL), `system_prompt` (TEXT NOT NULL DEFAULT ''), `variables` (JSONB NOT NULL DEFAULT '[]'), `enable_tools` (BOOLEAN NOT NULL DEFAULT false), `max_tokens` (INTEGER NOT NULL DEFAULT 4096), `version` (INTEGER NOT NULL DEFAULT 1), `created_at` (TIMESTAMPTZ), `updated_at` (TIMESTAMPTZ), `created_by` (UUID nullable), `updated_by` (UUID nullable)
2. A UNIQUE constraint exists on `(account_id, role, action)` — using a partial unique index to handle NULL account_id correctly: one index for `WHERE account_id IS NULL` and one for `WHERE account_id IS NOT NULL`
3. A `system_prompts` table exists with columns: `id` (UUID PK), `account_id` (UUID nullable), `role` (TEXT NOT NULL), `prompt` (TEXT NOT NULL), `version` (INTEGER NOT NULL DEFAULT 1), `created_at`, `updated_at`, `created_by`, `updated_by`
4. A UNIQUE constraint exists on `(account_id, role)` for `system_prompts` with the same partial index pattern
5. An `action_prompts` table exists with columns: `id` (UUID PK), `account_id` (UUID nullable), `action` (TEXT NOT NULL), `template` (TEXT NOT NULL), `variables` (JSONB NOT NULL DEFAULT '[]'), `enable_tools` (BOOLEAN NOT NULL DEFAULT false), `max_tokens` (INTEGER NOT NULL DEFAULT 4096), `version` (INTEGER NOT NULL DEFAULT 1), `created_at`, `updated_at`, `created_by`, `updated_by`
6. A UNIQUE constraint exists on `(account_id, action)` for `action_prompts` with the same partial index pattern
7. B-tree indexes exist on: `prompts(account_id)`, `prompts(role, action)`, `system_prompts(account_id)`, `system_prompts(role)`, `action_prompts(account_id)`, `action_prompts(action)`
8. Seed migration inserts 80 system default role+action templates (8 roles x 10 actions) from `default-prompts.ts` with `account_id = NULL`
9. Seed migration inserts 8 system prompts (one per role) with `account_id = NULL`
10. Seed migration inserts 10 action defaults (one per action) with `account_id = NULL`
...
### NULL vs Sentinel for account_id

System defaults use `account_id IS NULL` rather than the sentinel `DEFAULT_TENANT_ID` from Epic 17. This is intentional:
...
- Any other UUID = a specific account's overrides
...
### Partial Unique Indexes for NULL account_id

PostgreSQL treats `NULL != NULL` in unique constraints, so `UNIQUE(account_id, role, action)` would allow duplicate system defaults. The solution is two partial indexes:
...
CREATE UNIQUE INDEX idx_prompts_account_override
...
CREATE TABLE IF NOT EXISTS prompts (
  ...
  account_id    UUID REFERENCES tenants(id) ON DELETE CASCADE,
  role          TEXT NOT NULL,
...
```sql
INSERT INTO prompts (account_id, role, action, template, system_prompt, variables, enable_tools, max_tokens, version)
VALUES
...
```sql
INSERT INTO system_prompts (account_id, role, prompt, version)
VALUES
...
```sql
INSERT INTO action_prompts (account_id, action, template, variables, enable_tools, max_tokens, version)
VALUES
...
1. The `account_id` FK references `tenants(id)` from Epic 17. If Epic 17 is not yet deployed, the FK constraint can be deferred or the migration can add a `tenants` dependency check.
2. `ON DELETE CASCADE` on the `account_id` FK means deleting a tenant automatically deletes all its prompt overrides. System defaults (`account_id IS NULL`) are unaffected.
...
4. Partial unique indexes allow the same role+action for different account_ids
...
8. Insert a system default and an account override for the same role+action -- both rows exist
9. Delete a tenant -- verify CASCADE deletes account overrides but system defaults remain
10. `max_tokens <= 0` rejected by CHECK constraint
11. `version <= 0` rejected by CHECK constraint

## Dependencies

- **Epic 17** (Story 17-1: Tenant Model + Database Schema) -- the `tenants` table must exist for FK references
