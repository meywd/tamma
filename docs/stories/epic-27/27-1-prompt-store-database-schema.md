...
> **Annotation (Story 27-18 — taxonomy reshape).** The prompt-store **schema is
> unchanged** by Story 27-18. The C# port persists overrides in the single
> `prompt_overrides` table (see CLAUDE.md "Prompt Store Architecture / Storage")
> and keeps system defaults in code (`SystemPrompts.cs`), not in the
> `prompts`/`system_prompts`/`action_prompts` tables sketched below. What changed
> in 27-18 is the **seed content and action vocabulary**, now governed by the
> typed `(role, action)` taxonomy of **Story 27-15** (`RolePhaseMap` / `AgentRole`
> / `AgentAction`) and authoritatively regenerated by **Story 27-16** codegen:
> - The flat **8×10 = 80** role+action seed and the generic **10 action-default**
>   seed (AC 8 / AC 10 below) are **superseded** by the jagged per-role taxonomy
>   (~72 distinct action tokens; 85 `(role, action)` cells). There is **no
>   generic action-default tier** any more.
> - The action-vocabulary CHECK (AC 13) is now the 72-token `AgentAction` set.
> - Resolution is **tenant override → system default → `TammaError`** (fail-loud,
