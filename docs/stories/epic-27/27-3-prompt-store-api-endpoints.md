...
1. `GET /api/prompts` returns all resolved prompts for the authenticated user's account (account overrides merged with system defaults)
2. `GET /api/prompts/:role/:action` returns the resolved prompt for the current account (account override if exists, otherwise system default)
3. `PUT /api/prompts/:role/:action` creates or updates an account override for the current user's account; returns the updated prompt
4. `DELETE /api/prompts/:role/:action` deletes the account override (falls back to system default); returns 204 No Content
...
8. `POST /api/prompts/:role/:action/render` renders a prompt with variables for the current account (existing endpoint, now account-aware)
...
11. Account context is extracted from the authenticated user's session (from Epic 16/17 middleware)
...
- `GET /api/prompts` -- list all (no account scoping)
- `GET /api/prompts/:role/:action` -- get (no account scoping)
- `PUT /api/prompts/:role/:action` -- upsert (modifies global store)
- `POST /api/prompts/:role/:action/render` -- render with variables

These must be replaced with account-aware versions.

### Route Structure

```
/api/prompts                         GET     List resolved prompts (account merged with system)
/api/prompts/:role/:action           GET     Get resolved prompt
/api/prompts/:role/:action           PUT     Create/update account override
/api/prompts/:role/:action           DELETE  Delete account override
...
| `PUT /api/prompts/:role/:action` | Yes | Account admin or owner |
| `DELETE /api/prompts/:role/:action` | Yes | Account admin or owner |
...
| File | Purpose |
|------|---------|
| `packages/api/src/routes/prompts/prompt-routes.ts` | Rewritten with account-scoped routes |
...
Each route handler extracts `accountId` from the request context (set by auth middleware from Epic 16):
...
async function handleGetPrompt(
  ...
  const accountId = request.accountId ?? null; // from auth middleware
...
app.put('/api/prompts/:role/:action', { preHandler: [requireAccountAdmin] }, handleUpsertPrompt);
app.delete('/api/prompts/:role/:action', { preHandler: [requireAccountAdmin] }, handleDeletePrompt);
...
The render endpoint is updated to pass `accountId`:
...
async function handleRenderPrompt(request, reply) {
  ...
  const result = await store.render(accountId, role, action, { variables });
  if (result === undefined) {
...
1. The `accountId` is extracted from the request via `request.accountId` which is set by the auth middleware from Epic 16/17. For unauthenticated requests or CLI mode, `accountId` defaults to `null`, which resolves system defaults only.
...
3. The `DELETE /api/prompts/:role/:action` endpoint only deletes account overrides. System defaults cannot be deleted via this endpoint (only reset via the system endpoint).
...
1. `GET /api/prompts` returns merged list for authenticated account
2. `GET /api/prompts` returns system defaults for unauthenticated request
3. `GET /api/prompts/:role/:action` returns account override when it exists
4. `GET /api/prompts/:role/:action` returns system default when no override exists
5. `PUT /api/prompts/:role/:action` creates account override; returns 200
...
8. `DELETE /api/prompts/:role/:action` removes account override; returns 204
...
15. `POST /api/prompts/:role/:action/render` renders with account-scoped resolution

### Integration Tests

16. Full account override lifecycle: create, read, update, delete, verify fallback
17. Elsa workflow `ResolvePromptFromRegistryActivity` can call render endpoint with accountId

## Dependencies

- **Story 27-2** (Prompt Store Service) -- `IPromptStore` implementation must exist
- **Epic 16** (Story 16.5: RBAC Enforcement) -- auth middleware provides `request.accountId` and role checks
