...
1. An "AI Prompts" page is accessible from the tenant settings navigation under "Settings > AI Prompts"
2. The page displays all resolved prompts for the current tenant: system defaults with tenant overrides visually highlighted (e.g., badge, different background color, or icon)
3. Each prompt row shows: Role, Action, Source (System Default / Tenant Override), Version, Enable Tools, Max Tokens, Last Updated
...
5. When editing a system default, the panel shows a "This is a system default. Saving will create an tenant override." notice
6. The "Save" button calls `PUT /api/prompts/:role/:action` to create/update the tenant override
...
13. Only tenant admin or owner users can modify prompts; regular members see prompts as read-only
...
The dashboard knows the current user's account via the JWT token / session from Epic 16. The tenant ID is passed to the API automatically via the auth middleware.

### API Endpoints Consumed

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `GET /api/prompts` | GET | List resolved prompts (merged view for current tenant) |
| `GET /api/prompts/:role/:action` | GET | Get resolved prompt |
| `PUT /api/prompts/:role/:action` | PUT | Create/update tenant override |
| `DELETE /api/prompts/:role/:action` | DELETE | Remove tenant override |
| `POST /api/prompts/:role/:action/render` | POST | Render prompt with test variables |

### Difference from Admin UI (Story 27-4)

| Aspect | Admin UI (27-4) | Tenant UI (27-5) |
|--------|----------------|-------------------|
...
| File | Purpose |
|------|---------|
| `packages/dashboard/src/pages/tenant/PromptsPage.tsx` | Tenant prompts page |
| `packages/dashboard/src/components/prompts/TenantPromptTable.tsx` | Table with override highlighting |
| `packages/dashboard/src/components/prompts/TenantPromptEditor.tsx` | Editor with override-aware behavior |
...
| `packages/dashboard/src/hooks/useTenantPrompts.ts` | Data fetching hook for tenant prompts |
| `packages/dashboard/src/pages/tenant/PromptsPage.test.tsx` | Page tests |
...
| `packages/dashboard/src/components/navigation/TenantNav.tsx` (or equivalent) | Add "AI Prompts" link |

## Implementation Plan

### Step 1: Data Fetching Hook

Create `useTenantPrompts()` hook:

```typescript
function useTenantPrompts() {
...
The API response for `GET /api/prompts` must include a field indicating whether each prompt is an tenant override or a system default. This can be a boolean `isOverride` field or an `tenantId` field (non-null = override).
...
- If editing a system default: show info banner "This is a system default. Saving will create an override for your tenant."
- If editing an existing override: show "This is an tenant override." with "Reset to Default" button
...
1. The `GET /api/prompts` response needs to distinguish overrides from defaults. The API should include `source: 'system' | 'override'` or `tenantId: string | null` in each prompt summary. This requires a minor addition to the `list()` method's response format in Story 27-2/27-3.
...
1. `TenantPromptTable` renders prompts with correct source badges
...
5. `TenantPromptEditor` shows info banner for system defaults
6. `TenantPromptEditor` shows "Reset to Default" for overrides
...
| Task | Hours |
|------|-------|
| useTenantPrompts hook (data fetching) | 2 |
| TenantPromptTable component (table, badges, filters, count) | 3 |
| TenantPromptEditor component (editor, override-aware behavior) | 3 |
