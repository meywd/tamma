...
Create three PostgreSQL tables (`prompts`, `system_prompts`, `action_prompts`) with multi-tenant support via nullable `account_id`, partial unique indexes for NULL handling, and seed data generated from `packages/api/src/services/default-prompts.ts`. Uses the existing migration runner at `database/migrate.ts`.
...
**File to create**: `database/migrations/008_prompt_store.sql`

The next migration number is 008 (after `007_users_soft_delete.sql`).
...
-- =============================================================================
-- Table: prompts (80 system defaults + account overrides)
-- =============================================================================

CREATE TABLE IF NOT EXISTS prompts (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  account_id    UUID,  -- NULL = system default; FK added when tenants table exists
  role          TEXT NOT NULL,
...
-- Partial unique indexes (handle NULL account_id correctly)
CREATE UNIQUE INDEX IF NOT EXISTS idx_prompts_system_default
  ON prompts (role, action)
  WHERE account_id IS NULL;

CREATE UNIQUE INDEX IF NOT EXISTS idx_prompts_account_override
  ON prompts (account_id, role, action)
...
-- =============================================================================
-- Table: system_prompts (8 role preambles + account overrides)
-- =============================================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_system_prompts_account_override
  ON system_prompts (account_id, role)
  WHERE account_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_system_prompts_account_id ON system_prompts (account_id);
CREATE INDEX IF NOT EXISTS idx_system_prompts_role ON system_prompts (role);

-- =============================================================================
-- Table: action_prompts (10 action defaults + account overrides)
-- =============================================================================
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_action_prompts_account_override
  ON action_prompts (account_id, action)
  WHERE account_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_action_prompts_account_id ON action_prompts (account_id);
CREATE INDEX IF NOT EXISTS idx_action_prompts_action ON action_prompts (action);
```

**Note on FK constraints**: The `account_id` FK to `tenants(id)` is deferred until Epic 17 is deployed. When adding the FK later, use:
```sql
ALTER TABLE prompts ADD CONSTRAINT fk_prompts_account_id
 ...
 *
 * Usage: npx tsx scripts/generate-prompt-seed-sql.ts >> database/migrations/008_prompt_store.sql
 */
...
function main(): void {
  ...
  console.log('INSERT INTO prompts (account_id, role, action, template, system_prompt, variables, enable_tools, max_tokens, version)');
  console.log('VALUES');
  ...
  console.log('INSERT INTO system_prompts (account_id, role, prompt, version)');
  console.log('VALUES');
  ...
  console.log('INSERT INTO action_prompts (account_id, action, template, variables, enable_tools, max_tokens, version)');
  console.log('VALUES');
...
```bash
npx tsx scripts/generate-prompt-seed-sql.ts >> database/migrations/008_prompt_store.sql
```
...
# Verify row counts
psql $DATABASE_URL -c "SELECT COUNT(*) FROM prompts WHERE account_id IS NULL"       -- expect 80
psql $DATABASE_URL -c "SELECT COUNT(*) FROM system_prompts WHERE account_id IS NULL" -- expect 8
psql $DATABASE_URL -c "SELECT COUNT(*) FROM action_prompts WHERE account_id IS NULL" -- expect 10

# Verify idempotency (run again)
DATABASE_URL=postgres://... npx tsx database/migrate.ts
# Should print "skip: 008_prompt_store.sql (already applied)"
```
...
**File to create**: `database/migrations/__tests__/008_prompt_store.test.ts`
...
describe('Migration 008: Prompt Store', () => {
  ...
  it('should reject duplicate system default for same role+action', async () => {
    await expect(client.query(`
      INSERT INTO prompts (account_id, role, action, template) VALUES (NULL, 'developer', 'context-scan', 'dup')
    `)).rejects.toThrow(/unique/i);
  });

  // Test 3: Partial unique indexes allow same role+action for different accounts
  it('should allow same role+action for different account_ids', async () => {
    const acct1 = '11111111-1111-1111-1111-111111111111';
    const acct2 = '22222222-2222-2222-2222-222222222222';
    await client.query(`INSERT INTO prompts (account_id, role, action, template) VALUES ($1, 'developer', 'plan', 'acct1 override')`, [acct1]);
    await client.query(`INSERT INTO prompts (account_id, role, action, template) VALUES ($1, 'developer', 'plan', 'acct2 override')`, [acct2]);
    // Cleanup
    await client.query(`DELETE FROM prompts WHERE account_id IN ($1, $2)`, [acct1, acct2]);
  });

  // Test 4: Seed data row counts
  it('should seed 80 system default prompts', async () => {
    const result = await client.query(`SELECT COUNT(*)::int as cnt FROM prompts WHERE account_id IS NULL`);
    expect(result.rows[0]!['cnt']).toBe(80);
  });

  it('should seed 8 system prompts', async () => {
    const result = await client.query(`SELECT COUNT(*)::int as cnt FROM system_prompts WHERE account_id IS NULL`);
    expect(result.rows[0]!['cnt']).toBe(8);
  });

  it('should seed 10 action defaults', async () => {
    const result = await client.query(`SELECT COUNT(*)::int as cnt FROM action_prompts WHERE account_id IS NULL`);
  ...
  it('should reject max_tokens <= 0', async () => {
    await expect(client.query(`
      INSERT INTO prompts (account_id, role, action, template, max_tokens)
  ...
  it('should reject version <= 0', async () => {
    await expect(client.query(`
      INSERT INTO prompts (account_id, role, action, template, version)
...
| # | File Path | Purpose |
|---|-----------|---------|
| 1 | `database/migrations/008_prompt_store.sql` | DDL + seed data |
| 2 | `scripts/generate-prompt-seed-sql.ts` | One-time seed SQL generator |
| 3 | `database/migrations/__tests__/008_prompt_store.test.ts` | Integration tests |
...
| Risk | Mitigation |
|------|------------|
| `tenants` table does not exist yet (Epic 17) | Omit FK constraint on `account_id` for now; add it via a future migration when tenants table is deployed |
