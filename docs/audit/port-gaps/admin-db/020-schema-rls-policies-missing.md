...
- **Notes**: Policies + role exist in the migration but are not enforced at runtime. `20260419021119_Phase2RlsAndTriggers` migration installs `ENABLE ROW LEVEL SECURITY` + `FORCE ROW LEVEL SECURITY` + a `tenant_isolation_policy` on 14 tenant-scoped tables: `tenants`, `tenant_memberships`, `users`, `github_installations`, `github_installation_repos` (joined via parent), `user_invites`, `api_keys` (with service-scope cross-tenant exemption), `domain_events`, `workflow_instances`, `workflow_definitions`, `agent_configs`, `provider_diagnostics`, `provider_health`, `sanitization_rules`, `prompt_overrides`. Policies use `current_setting('app.current_tenant_id', true)::uuid` matching the existing `TenantContextMiddleware`.
- **Why "scaffold only"**: RLS only fires against sessions that are NOT superusers. The runtime connects via `TammaDbContext`, which uses the admin (superuser) connection string; superusers bypass RLS. `TammaAppDbContext` exists but is not injected anywhere in production code — zero repositories and zero endpoints consume it. Full activation requires threading `TammaAppDbContext` through per-request endpoints and repositories — tracked as follow-up story `docs/stories/epic-19/story-19-6-wire-app-role-context.md`.
- **Scaffold shape (unchanged)**: **Phase-3 (2026-04-18, commits e53c5a1 / 9e20e05 / 159f12a)**: The new `TammaAppDbContext` (app-role subclass) + `TenantContextInterceptor` (runs `set_config('app.current_tenant_id', ...)` on connection open) + the dedicated `ConnectionStrings:TammaAppDb` let the per-request plane connect as `tamma_app`, which is NOT a superuser and therefore honors RLS — when it is actually used. Integration tests (`Tamma.Api.Tests/Tenancy/QueryFilterAndInterceptorTests`) prove cross-tenant reads are denied under `tamma_app` + the bound tenant, and verify superuser connections still bypass policies as expected. Per-request endpoints migrate from `TammaDbContext` to `TammaAppDbContext` as the follow-up; day-1 runtime keeps using `TammaDbContext` (admin role) so existing behavior is unchanged until each repository is flipped.
