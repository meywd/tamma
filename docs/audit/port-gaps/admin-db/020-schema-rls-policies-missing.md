...
- **Outcome**: Fixed (policies installed + activated on the app-role plane in Phase-3)
- **Notes**: `20260419021119_Phase2RlsAndTriggers` migration installs `ENABLE ROW LEVEL SECURITY` + `FORCE ROW LEVEL SECURITY` + a `tenant_isolation_policy` on 14 tenant-scoped tables: `tenants`, `tenant_memberships`, `users`, `github_installations`, `github_installation_repos` (joined via parent), `user_invites`, `api_keys` (with service-scope cross-tenant exemption), `domain_events`, `workflow_instances`, `workflow_definitions`, `agent_configs`, `provider_diagnostics`, `provider_health`, `sanitization_rules`, `prompt_overrides`. Policies use `current_setting('app.current_tenant_id', true)::uuid` matching the existing `TenantContextMiddleware`. **Phase-3 update (2026-04-18, commits e53c5a1 / 9e20e05 / 159f12a)**: RLS is no longer dormant. The new `TammaAppDbContext` (app-role subclass) + `TenantContextInterceptor` (runs `set_config('app.current_tenant_id', ...)` on connection open) + the dedicated `ConnectionStrings:TammaAppDb` let the per-request plane connect as `tamma_app`, which is NOT a superuser and therefore honors RLS. Integration tests (`Tamma.Api.Tests/Tenancy/QueryFilterAndInterceptorTests`) prove cross-tenant reads are denied under `tamma_app` + the bound tenant, and verify superuser connections still bypass policies as expected. Per-request endpoints migrate from `TammaDbContext` to `TammaAppDbContext` per finding as a follow-up; day-1 runtime keeps using `TammaDbContext` (admin role) so existing behavior is unchanged until each repository is flipped.
