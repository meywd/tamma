...
- **Notes**: Scaffold landed in commits `e53c5a1`, `9e20e05`, `159f12a` (TammaAppDbContext, TenantContextInterceptor, fail-closed filter). BUT: the interceptor is only effective on a `TammaAppDbContext`-backed connection connecting as the `tamma_app` role, and zero production code paths inject `TammaAppDbContext` — all 21 repositories and every endpoint still resolve `TammaDbContext` (admin role). The admin role bypasses RLS because superusers skip policies, so the interceptor's `set_config` call is a no-op at the defense-in-depth layer even though the plumbing works. Full remediation requires wiring tenant-scoped repositories onto the app-role connection — tracked as follow-up story in `docs/stories/epic-19/story-19-6-wire-app-role-context.md`.
