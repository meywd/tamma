...
- **Outcome**: Fixed
- **Commit**: `4e1e0e4`
- **Notes**: Real `LibsodiumGitHubSecretsProvisioner` landed using `Sodium.Core` 1.4.0 (libsodium-backed; `SealedPublicKeyBox.Create` wraps `crypto_box_seal` X25519 + XSalsa20-Poly1305). Flow: fetch repo public key via `Repository.Actions.Secrets.GetPublicKey`, sealed-box encrypt, standard base64 encode (GitHub's wire format), PUT via `Repository.Actions.Secrets.CreateOrUpdate`. Concurrency capped at 5 parallel writes via `SemaphoreSlim` (matches the TS `MAX_CONCURRENCY`). Per-repo failures do not abort the batch — archived repos, forbidden responses, and rate-limit exceptions all surface as `SecretProvisionResult` entries with explanatory error strings. Both callers (`InstallationRouterService.IssueInstallationKeyAsync` + `ApiKeyRotationService.RotateInternalAsync`) auto-promote to real provisioning when the GitHub App client is wired. Covered by `LibsodiumGitHubSecretsProvisionerTests` — sealed-box round-trip, ciphertext uniqueness (ephemeral-keypair property), standard base64 output, UTF-8 plaintext, malformed-key rejection. Chose `Sodium.Core` over `NSec.Cryptography` for direct API parity with the TS `libsodium-wrappers` test vectors and simpler sealed-box surface.
