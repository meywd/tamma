...
- ✅ **Wave 4 — Final cross-cutting review fixes:** `9d42b25c` (I-1 audit events — `ConventionEventsService` mirroring prompt precedent, wired to all 5 mutation endpoints; new event-type constants; mutation signatures extended with `wasCreated`/`wasDeleted`), `5e1fd1ea` (I-3 idempotent DI, I-4 single-roundtrip resolve via enriched `ConventionResolution`, I-5 shared `RoleActionParsing.TryParsePair` for prompt+convention endpoints — prompt 404→400 contract change on ineligible pairs, I-6 PUT null guard, M-1/M-2 polish), `17e235e5` (taxonomy-neutral error codes `INVALID_ROLE_ACTION`/`INELIGIBLE_ROLE_ACTION`; `ResetSystemDefaultAsync` returns row + drops trailing `GetAsync`; collapsed `TenantScope` to one helper).
- ✅ **Wave 5 — Story 27-13** (Elsa `ResolveConventionsActivity`): `5202a56b` (new activity in `src/Tamma.Activities/Context/` mirroring `ResolvePromptFromRegistryActivity` — POST to `/api/conventions/resolve` with `X-Tenant-Id`; Parse-at-boundary; empty-action legacy passthrough TODO(27-19); auto-emits `LLM.CONVENTIONS.RESOLVE.{STARTED,COMPLETED,FAILED}`; wired between `InitInputs` and `ResolvePrompt` in `LlmCallWorkflow` with `MergeConventions` SetVariable), `fef3e51a` (review fixes — 5xx→`REGISTRY_UNAVAILABLE` retryable in BOTH activities, prompt code renamed `REGISTRY_MISS`→`NO_ROW` for symmetry, `version` surfaced in COMPLETED event, X-Tenant-Id moved from `DefaultRequestHeaders` to request msg + tested).
- ✅ **Wave 5 — Story 27-14** (convention event sourcing refactor): `558b79d3` (moved emission from endpoints INTO `ConventionStore`; unified to 4 event types `CONVENTION.CREATED/UPDATED/DELETED/RESET.SUCCESS` with `source` tag distinguishing tenant vs system; `changedFields` body/enabled diff; `previousVersion`/`newVersion`/`deletedVersion` data; repo signatures returning `PreviousRow` + `DeletedVersion`), `d4a3caf5` (critical fix — `IConventionStore.DeleteAsync`/`DeleteSystemDefaultAsync` now require `actorUserId` so DELETE events carry real userId; endpoint test pins the propagation; CREATED.`enabled` + RESET.`resetFrom`/`resetTo` data fields per AC; `changedFields` ordering pinned with positional assertion).
- ✅ **Wave 5 final-review polish + deferred-items bundle:** `e038a87b` (**CRITICAL** — engine→API Bearer auth in both resolve activities via named HttpClient `"tamma-engine"` + `TammaEngineAuthHandler` reading `Tamma:ApiToken`, mirroring `TammaApiClient`; production blocker resolved), `4a06d747` (task #13 — 23505 unique-violation → 409 Conflict on both prompt + convention upsert endpoints), `fbf73e78` (IMP-1 — `MergeConventions` fails loud on malformed `variablesJson` instead of silently dropping other variables), `d0ecb9c3` (IMP-3 — RESET event's `resetTo` renamed `"hardcoded"`→`"code-baseline"` to reflect the DB-managed model), `da249c8a` (task #10 — `RunOnStartup` gates added to `OutboxSmtpSender`/`TaskQueueProcessor`/`ProviderSessionCleanupService`/`EngineRegistryHeartbeatService` + shared `ApiTestFixture` opts them off; fixes the `AuthRegisterTxnId` flake at root), `4ecd2ca1` (task #12 — Epic 28 cutover hazards documented on `IConventionStore`/`IConventionRepository`/`Convention` entity with grep markers `I-2`/`X-1` for fanout + tenant-id defence work).
- ✅ **Wave 6 — UIs** (frontend, `packages/dashboard`): `6265a8e6` (**task #4** — fix 27-18 dashboard breakage: deleted `ActionDefaultsList.tsx`, removed `actionDefaults` from prompts client/hook/page/tests), `ea12d3b5` (**Story 27-11** — convention admin UI: `ConventionsAdminPage` + `ConventionTable` + `ConventionEditor` + `ResolutionTestPanel` + `RoleActionSelector` + `MarkdownEditor` + admin-conventions client/hook; mounted under `/admin/conventions` with `AdminGuard`), `e1a5dd17` (**Story 27-12** — convention tenant UI: `ConventionsPage` (settings) + `TenantConventionTable` (override-highlighted) + `TenantConventionEditor` + `ConventionDiff` (custom LCS, no external lib) + `OverrideBadge` + `useTenantConventions`). React 19 + Vite + Tailwind. **42 test files / 269 tests passing**; clean build; no new deps.
