...
**Next — UIs only (Wave 6):** **27-11/27-12** (convention management UIs — frontend; also must address the 27-18 dashboard action-default breakage). Backend complete; UIs need UX direction.

**Tracked follow-ups (NOT done — defer to appropriate time):**
- Task #4: dashboard `packages/dashboard` action-default breakage (27-18) → governed by 27-11/27-12.
- Task #10: pre-existing flaky `StopgapSecretMigratorTests` under parallel load (shared audit state).
- Task #12: Epic 28 prep — convention admin/repo per-tenant-DB cutover hazards (I-2 + X-1).
- Task #13: map concurrent upsert 23505 → 409 Conflict (X-3, both prompt + convention).
- Management-list-shows-disabled-cells is a deferred UI decision (27-10 `GET /api/conventions` is the resolved view per AC1).
- Engine→API authentication review across all `Engine:CallbackUrl`-using activities (raised in 27-13 review).
- Prompt-store emission alignment: move prompt event emission from endpoints into `PromptStoreService` to match the 27-14 pattern.
