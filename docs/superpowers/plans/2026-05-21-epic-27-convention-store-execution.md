...
**Next — Wave 4:** **27-9** (convention service, C4/C5 — mirror `PromptStoreService`: parallel single-user `userId` + SaaS `…ForTenant(tenantId)` methods with DISTINCT names; per-tenant DB via `ITenantDbContextFactory`; resolve tenant→system→`TammaError` (no empty/plain); MUST validate `(role,action)` through `AgentRole.Parse`/`AgentAction.Parse` against `RolePhaseMap.EligibleActions`, same vocabulary as prompts) → **27-10** (API — mirror `PromptEndpoints`, new `ConventionManage` policy, member→403; keep legacy `/api/convention-templates` untouched) → **27-13** (Elsa `ResolveConventionsActivity` over HTTP, C2 — needs 27-10's resolve endpoint) ∥ **27-14** (events) → **27-11**/**27-12** (UIs).
